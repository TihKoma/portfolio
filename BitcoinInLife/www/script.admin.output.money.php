<?php
	...
	if(isset($_POST['action']) && $_POST['action'] == 'delete'){
		...
		for($i = 0; $i < count($mas); $i++){
			$id = '\''.$mas[$i].'\'';
			$q = mysql_query("DELETE FROM output_money WHERE id = $id");
			if($q != NULL)
				$sum += 1;
		};
		if($sum == $count)
		...
		$t = mysql_query("UPDATE output_money SET status = 1 WHERE id = $id");

		$login = get_login($uid);
		$action = "Подтверждён запрос на снятие $. Списано \$$count с аккаунта $login";
		$action = '\''.$action.'\'';
